...
import { updateQuote } from "@/lib/helpers/data.helpers"
import {
  UpdateQuoteFormInput,
  updateQuoteFormInputSchema,
} from "@/lib/schemas/quote.schema"
import { cn } from "@/lib/utils"

export function EditQuoteForm({ quote }: { quote: QuoteWithRecords }) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)

  const initialValues: UpdateQuoteFormInput = {
    id: quote?.id ?? "",
    type: quote?.type ?? "1",
  ...
  const form = useForm<UpdateQuoteFormInput>({
    resolver: zodResolver(updateQuoteFormInputSchema),
    values: initialValues,
  })

  async function onSubmit(values: UpdateQuoteFormInput) {
    try {
      setLoading(true)
